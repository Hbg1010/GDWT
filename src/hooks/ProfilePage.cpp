...
void GDWTProfilePage::loadPageFromUserInfo(GJUserScore* user) {
    ...
    m_fields->playerDataListener.spawn(
        data::getPlayersData(),
        [&](auto out){
            this->playersDataRecieved(std::move(out));
        }
    );
}

void GDWTProfilePage::playersDataRecieved(PlayerDataFuture::Output out){
    if (out.isErr()) return;

    auto playerData = out.unwrap();
